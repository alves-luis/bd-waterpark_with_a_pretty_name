...
CREATE (:Atracao {id: toInteger(row.Id), designacao: row.Designacao, zona: row.Zona, capacidade: toInteger(row.Capacidade), duracao: row.Duracao, altura_minima: toInteger(row.Altura_Minima)});
